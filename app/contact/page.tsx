    ...
    <main className="relative min-h-screen bg-black text-white selection:bg-primary selection:text-black overflow-x-hidden">
      ...
      <section className="px-8 md:px-16 grid grid-cols-1 lg:grid-cols-12 gap-0 border-y border-primary/20">
        ...
        >
          ...
          <form className="space-y-12">
            {[
              { id: "contact-name", label: "NAME / ENTITY", placeholder: "WHO ARE YOU?", type: "text" },
              { id: "contact-email", label: "ELECTRONIC MAIL", placeholder: "WHERE DO WE REPLY?", type: "email" },
              { id: "contact-mission", label: "THE MISSION", placeholder: "TELL US ABOUT THE KINETICISM YOU NEED.", type: "textarea" }
            ].map((field, i) => (
              <m.div 
                key={field.label} 
                ...
                className="group"
              >
                <label htmlFor={field.id} className="block text-xs font-bold uppercase tracking-widest text-white/60 mb-4">{field.label}</label>
                {field.type === "textarea" ? (
                  <textarea 
                    id={field.id}
      ...
      <section className="w-full h-[600px] bg-surface relative overflow-hidden">
        ...
        <div className="absolute bottom-16 left-16">
          <p aria-hidden="true" className="font-headline text-9xl font-black uppercase tracking-tighter opacity-10 select-none">REALM</p>
        </div>
      </section>
